...
function validateNumber() {
	...
	var isValid = /^[0-9]{10}$/.test(number.value);
	color(isValid, number);
    return isValid;
}

function validateForm() {
    if(validateFirstName() && validateLastName() && validateEmail() && validatePassword() && validateConfirmPassword() && validateNumber() && validateDOB()) {        
        //alert("Submitting");
        return true;
    }
    else {
